...
default) was applied to the STRING interaction network to partition the
45 MAL-associated proteins into biologically coherent modules. MCL
was run using the \texttt{MCL} R package (\texttt{mcl()},
\texttt{addLoops=TRUE}, \texttt{inflation=2}) on the binary adjacency
matrix of the simplified subnetwork. Proteins not assigned to any
cluster (cluster 0, $n=17$) were excluded. Clusters with fewer than 3
proteins were also excluded from enrichment analysis. For each retained
cluster, pathway enrichment was retrieved from the STRING API across
Gene Ontology (GO:BP, GO:MF, GO:CC), KEGG, and Reactome databases;
FDR was computed by STRING using the Benjamini--Hochberg procedure
against the human proteome background. Full results are in
\texttt{STRING\_mcl\_clusters.csv} and
\texttt{STRING\_cluster\_enrichment.csv}.

Four clusters with $\geq 3$ proteins emerged (covering 22 of 45
proteins assigned; 17 unassigned): a cytokine/hormone signaling
cluster ($n=9$), a xenobiotic/alcohol metabolism cluster ($n=5$), a
cell adhesion and heparan sulfate proteoglycan cluster ($n=5$), and an
ADAMTS metalloproteinase cluster ($n=3$).

\begin{table}[H]
\centering
\caption*{\textbf{MCL cluster enrichment summary.} Top enriched pathway
  per cluster (lowest FDR across GO:BP, KEGG, and Reactome).
  Clusters with $\geq 3$ proteins are shown.
  KEGG = Kyoto Encyclopedia of Genes and Genomes;
  RCTM = Reactome; GO:CC = Gene Ontology Cellular Component.
  Full results in \texttt{STRING\_cluster\_enrichment.csv}.}
\begin{tabular}{clp{5.2cm}p{3.5cm}rr}
\toprule
Cluster & $n$ & Members & Top pathway & DB & FDR \\
\midrule
1 & 9 & \small ADIPOQ, CXCL12, GHR, IGFBP1, IGFBP2, IL5RA, ITLN1, LEP, SERPINA11
  & Cytokine-cytokine receptor interaction & KEGG & 0.0018 \\[4pt]
2 & 5 & \small ADH1A, ADH1C, ADH4, ADH7, GSTA1
  & Metabolism of xenobiotics by cytochrome P450 & KEGG & $1.6 \times 10^{-10}$ \\[4pt]
3 & 5 & \small AGRN, HS6ST3, NCAN, PAK7, RTN4R
  & Heparan sulfate/heparin (HS-GAG) metabolism & RCTM & $5.3 \times 10^{-4}$ \\[4pt]
4 & 3 & \small ADAMTS5, ADAMTSL2, TIMP4
  & Extracellular matrix & GO:CC & 0.034 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption*{\textbf{STRING network with MCL cluster assignments.}
  Nodes represent the 45 MAL-associated proteins (FDR $< 0.05$);
  edges represent STRING interactions (combined score $\geq 400$).
  Node colours indicate MCL cluster membership (inflation = 2).
  Four clusters with $\geq 3$ proteins are highlighted; 17 proteins
